...
enum caut_status encode_bool(P * const iter, bool const * const obj) {
  CAUT_ASSERT(iter);
  CAUT_ASSERT(obj);
  MEMMOVE_PACK(iter, (unsigned char*)obj);
}
...
GENERIC_UNPACK(u8,  unsigned char)
...
enum caut_status decode_bool(U * const iter, bool * const obj) {
  CAUT_ASSERT(iter);
  CAUT_ASSERT(obj);
  MEMMOVE_UNPACK(iter, (unsigned char*)obj);
}
...
enum caut_status __caut_encode_raw_bytes(struct caut_encode_iter * const iter, unsigned char const * const bytes, size_t len) {
...
enum caut_status __caut_decode_raw_bytes(struct caut_decode_iter * const iter, unsigned char * const bytes, size_t len) {
...
enum caut_status encode_tag8(struct caut_encode_iter * const iter, unsigned char const * const obj) {
...
enum caut_status decode_tag8(struct caut_decode_iter * const iter, unsigned char * const obj) {
...
enum caut_ord compare_u8(unsigned char const * a, unsigned char const * b) { return CAUT_ORDER(*a, *b); }
...
void init_u8(unsigned char * o) { *o = 0; }
