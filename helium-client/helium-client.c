...
static size_t
_read_frame(struct helium_ctx * ctx, unsigned char * frame, size_t len)
{
    enum decode_state state            = sof;
    size_t            payload_size     = 0;
    unsigned char           payload_checksum = 0;
...
static size_t
_write_frame(struct helium_ctx * ctx, unsigned char * frame, size_t len)
{
    if (len == 0)
    {
        return 0;
...
_mk_token(unsigned char frame_id, unsigned char channel_ref)
{
...
{
	unsigned char * frame       = ctx->buf;
	unsigned char   channel_ref = ctx->channel_ref++;
